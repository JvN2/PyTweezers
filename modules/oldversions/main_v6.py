...
from modules.TweezerImageProcessing import (
    proces_image,
    Beads,
    Traces,
    get_roi,
...
def test_acquisition(settings, image=None, show=False):
    ...
    start = time()
    for i in tqdm(
        range(n_images), postfix="Acquiring images: Simulation using saved image"
    ):
        results.append(proces_image(i, image, settings))
    print(
        color_text(0, 100, 0, f"Computed in {n_images / (time() - start):.3f} frames/s")
    )
    df = convert_result(
        results, pars=["frame", "%: X (pix)", "%: Y (pix)", "%: Z (um)", "%: A (a.u.)"]
    )
    print(df.tail(4))
