...
def test_acquisition(acquisition_settings, processing_settings, image=None):
    ...
    def aquire_images(settings, buffer_index, n_frames=100, image=None):
        ...
        if image is None:
            ...
            for _ in tqdm(range(n_frames), postfix="Acquiring images"):
                ...
                rois = [
                    get_roi(frame, tracker.roi_size, coord)
            ...
            for frame_index in tqdm(
                ...
                rois = [
                    get_roi(image, settings["size"], coord)
                    for coord in settings["coords"]
                ]
    ...
    def process_rois(settings, buffer_index, processed_data, identifier):
        ...
        while True:
            ...
            result = np.asarray(
                np.append([item["frame"], identifier], np.reshape(result, (-1)))
            )
    ...
    threads = [
        Thread(
            target=process_rois,
            args=(processing_settings, buffer_index, processed_data, i),
        )
        for i in range(n_threads)
    ]
    threads.append(
        Thread(
            target=aquire_images,
            args=(acquisition_settings, buffer_index, n_images, image),
        )
    )
    t_start = time()
