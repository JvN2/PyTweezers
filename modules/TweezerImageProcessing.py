...
def get_peak(im):
    ...
    intensity[0] = (
        im[position[0], peak[1]]
        if position[0] >= 0
        else im[position[0] + shape[0], peak[1]]
    )
    intensity[1] = im[position[1], peak[1]]
    intensity[2] = (
        im[position[2], peak[1]]
        if position[2] < shape[0]
        else im[position[2] - shape[0], peak[1]]
    )
    x = calc_extreme(position, intensity)

    position = peak[1] + offset
    intensity[0] = (
        im[peak[0], position[0]]
        if position[0] >= 0
        else im[peak[0], position[0] + shape[0]]
    )
    intensity[1] = im[peak[0], position[1]]
    intensity[2] = (
        im[peak[0], position[2]]
        if position[2] < shape[1]
        else im[peak[0], position[2] - shape[1]]
    )
...
class Beads:
    ...
    def save_avi(self, movie, filename, fps=15):
        ...
        out = cv2.VideoWriter(
            str(filename), fourcc, fps, movie[0].T.shape, isColor=False
        )
    ...
    def find_focus(self, lut):
        ...
        high_frequencies = np.asarray(
            [
                np.percentile(
                    np.abs(calc_fft(l)) * create_filter(size, size * 0.45), 95
                )
                for l in lut
            ]
        )
        high_frequencies /= np.asarray(
            [
                np.percentile(
                    np.abs(calc_fft(l)) * create_filter(size, size * 0.85), 95
                )
                for l in lut
            ]
        )
    ...
    def resample_lut(self, z, lut, n_points, z_width=1):
        ...
        if z[-1] < z[0]:
            new_z = new_z[::-1]
        new_lut = [
            np.sum(lut * calc_weight(z, z0, z_width)[:, np.newaxis, np.newaxis], axis=0)
            for z0 in new_z
        ]
    ...
    def _create_lut(self, filename, highpass=20, lowpass=90, z_width=0.25):
        ...
        if ".tdms" in filename.suffix:
            z_calib = (
                np.asarray(TdmsFile(filename)["Tracking data"]["Focus (mm)"]) * 1000
            )
            ...
            try:
                ...
                coords = self.pick_beads(
                    movie[0], n_beads=1, title="Pick bead for LUT", filename=lut_file
                )
                lut = np.asarray(
                    [get_roi(im, self.roi_size, coords[0])["image"] for im in movie]
                ).astype(np.uint8)

                focus_index = self.find_focus(
                    lut
                )  # find the frame with the highest spatial frequencies
                ...
                with pd.ExcelWriter(lut_file.with_suffix(".xlsx")) as writer:
                    pd.DataFrame(z_calib, columns=["z_calib (um)"]).to_excel(
                        writer, sheet_name="traces", index=True, float_format="%.5f"
                    )
                    pd.DataFrame(coords, columns=["x0 (pix)", "y0 (pix)"]).to_excel(
                        writer, sheet_name="parameters"
                    )
                    globs = pd.DataFrame(
                        [["roi_pix", self.roi_size, "settings"]],
    ...
    def _resample_lut(self, z_calib, lut, width_um=0.3):
        z_array = np.linspace(
            np.min(z_calib),
            np.max(z_calib),
            int((np.max(z_calib) - np.min(z_calib)) / width_um),
        )
        new_lut = [
            np.sum(
                multiply_along_axis(lut, calc_weight(z_calib, z, width_um), 0), axis=0
            )
            for z in z_array
        ]
    ...
    def pick_beads(self, image, filename=None, n_beads=None, title=None):
        ...
        while True and len(coords_list) < n_beads:
            ...
            try:
                ...
                box = plt.Rectangle(
                    [c[0] - self.roi_size / 2, c[1] - self.roi_size / 2],
                    self.roi_size,
                    self.roi_size,
                    edgecolor=color,
                    facecolor="none",
    ...
    def find_beads(self, im, n_max=100, treshold=None, show=False):
        ...
        for period in tqdm(
            [38.4, 29.3, 47.5, 52.0], postfix="Cross correlating with ref images"
        ):
            ref_im = create_ref_image(period, shape=im.shape, width=period * 10)
            cc *= np.abs(
                np.fft.ifft2(mask * np.fft.fft2(im) * np.conjugate(np.fft.fft2(ref_im)))
            )
        ...
        coords_list = []
        for _ in tqdm(
            range(n_max), postfix="find_beads: Finding peaks in cross-corelation"
        ):
        ...
        coords = pd.DataFrame(
            coords_list,
            columns=["X0 (pix)", "Y0 (pix)", "width (pix)", "amplitude (a.u.)", "R2"],
        )
        ...
        if show:
            ...
            for i, row in coords.iterrows():
                ...
                # if color == 'green':
                box = plt.Rectangle(
                    (row["X0 (pix)"] - roi_size / 2, row["Y0 (pix)"] - roi_size / 2),
                    roi_size,
                    roi_size,
                    edgecolor=color,
                    facecolor="none",
                ...
                plt.text(
                    row["X0 (pix)"] - roi_size / 2,
                    row["Y0 (pix)"] + roi_size / 1.9,
                    f'{i}: {row["R2"]:.2f}',
                    color=color,
                )
            plt.show()
    ...
    def process_image(self, im, coords=None):
        if coords is None:
            coords = self.coords
        result = np.asarray(
            [self.get_xyz(get_roi(im, self.roi_size, c)["image"], c) for c in coords]
        ).astype(float)
        return result
...
class Traces:
    ...
    def sort_traces(self):
        reordered_cols = np.append(
            [c for c in self.traces.columns if ": " not in c],
            natsorted(
                [c for c in self.traces.columns if ": " in c],
    ...
    def read_log(self, filename=None):
        ...
        self.globs = pd.DataFrame(
            log, columns=["parameter", "value", "section"]
        ).set_index("parameter")
    ...
    def add_frame_coords(self, frame, coords):
        ...
        for i, bead in enumerate(coords):
            names.extend(
                [f"{i}: x (pix)", f"{i}: y (pix)", f"{i}: z (um)", f"{i}: a (a.u.)"]
            )
        try:
