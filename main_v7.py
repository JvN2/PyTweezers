...
def framegrabber_close(cam, queue):
    ...
    df = get_queue(queue, pars=['frame', 'thread', '%: X (pix)', '%: Y (pix)', '%: Z (um)', '%: A (a.u.)'])
    return df
...
def grab_and_process(cam, settings, queue, i):
    while True:
        ...
        result = [get_xyza(roi, settings['freqs'], settings['lut'], settings['lut_z_um'], filter=settings['filter'])
                  for roi in rois]
        result = np.asarray(np.append([info['frame_index'], i], np.reshape(result, (-1))))
        queue.put(result)
    return
...
def test_multi_processing(settings, image, show=False):
    ...
    processes = [Process(target=grab_and_process, args=(cam, settings, queue, i,)) for i in range(n_cores)]
